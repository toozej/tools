...
- You work on small, well‑scoped changes across multiple apps and shared infrastructure
...
- **App Types:**
  - **Static apps:** Next.js with `output: 'export'` (e.g., taboo, unit-converter, yaml-formatter) — built once, served by nginx
  - **Server apps:** Next.js with API routes/OAuth (e.g., gh-dashboard, clip2gist, www2s) — long-running Node.js services
  - **Go services:** Long-running services proxied via nginx backend network
  - **HTML apps:** Pure client-side apps served by nginx
- **File Structure:**
  - `apps/` – Individual applications (you READ and WRITE here per-app)
  - `templates/` – Dockerfile and .dockerignore templates for new apps
  - `nginx/` – nginx configuration
  - `scripts/` – Utility scripts (e.g., Next.js upgrade helpers)
  - `README.md` – Project documentation

### Homepage app (`apps/homepage`)
The homepage is the centerpiece of the tools repo — it's the main entry point for users to discover and access all tools.
...
- **Data Source:** [`apps/homepage/src/data/colophon.json`](apps/homepage/src/data/colophon.json) — generated by `uv run generate-colophon --output ./apps/homepage/src/data/colophon.json`
- **Key Features:**
...
- Respect app folder boundaries — only modify files within the relevant `apps/<app>/` directory
