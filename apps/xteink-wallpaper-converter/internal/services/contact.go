...
// BMP encoding for 24-bit uncompressed format
func encodeBMP(img image.Image) ([]byte, error) {
	bounds := img.Bounds()
	width := bounds.Dx()
	height := bounds.Dy()

	// Calculate row size (24-bit = 3 bytes per pixel, padded to 4-byte boundary)
	rowSize := (width*3 + 3) & ^3
	imageSize := rowSize * height
	fileSize := 54 + imageSize // Header (54 bytes) + image data
	...
	copy(buf[offset:], "BM")
	offset += 2
	binary.LittleEndian.PutUint32(buf[offset:], uint32(fileSize)) // File size
	offset += 4
	binary.LittleEndian.PutUint32(buf[offset:], 0) // Reserved
	offset += 4
	binary.LittleEndian.PutUint32(buf[offset:], 54) // Data offset
	offset += 4

	// BMP Info Header (40 bytes)
	binary.LittleEndian.PutUint32(buf[offset:], 40) // Header size
	offset += 4
	binary.LittleEndian.PutUint32(buf[offset:], uint32(width)) // Width
	offset += 4
	binary.LittleEndian.PutUint32(buf[offset:], uint32(height)) // Height
	offset += 4
	binary.LittleEndian.PutUint16(buf[offset:], 1) // Planes
	offset += 2
	binary.LittleEndian.PutUint16(buf[offset:], 24) // Bits per pixel
	offset += 2
	binary.LittleEndian.PutUint32(buf[offset:], 0) // Compression
	offset += 4
	binary.LittleEndian.PutUint32(buf[offset:], uint32(imageSize)) // Image size
	offset += 4
	binary.LittleEndian.PutUint32(buf[offset:], 2835) // X pixels per meter
	offset += 4
	binary.LittleEndian.PutUint32(buf[offset:], 2835) // Y pixels per meter
	offset += 4
	binary.LittleEndian.PutUint32(buf[offset:], 0) // Colors used
	offset += 4
	binary.LittleEndian.PutUint32(buf[offset:], 0) // Important colors
	offset += 4

	// Image data (bottom-up, BGR format)
...
// ConvertImageToWallpaper converts an image to XTeink wallpaper format (480x800 BMP)
func ConvertImageToWallpaper(imgData []byte, cropX, cropY, cropWidth, cropHeight int) ([]byte, error) {
	img, _, err := image.Decode(bytes.NewReader(imgData))
	if err != nil {
		return nil, errors.New("failed to decode image")
	}
	...
	// If no crop specified, use automatic cropping
	if cropWidth == 0 || cropHeight == 0 {
		// Calculate crop to fit 480x800 aspect ratio, prioritizing full height usage
		targetAspect := 480.0 / 800.0 // 0.6

		// Prioritize using full height when possible
		cropH := imgH
		cropW := int(float64(cropH) * targetAspect)

		// If calculated width exceeds image width, use full width instead
		if cropW > imgW {
			cropW = imgW
	...
	if cropX+cropWidth > imgW {
		cropWidth = imgW - cropX
	}
	if cropY+cropHeight > imgH {
		cropHeight = imgH - cropY
	}
	...
	if si, ok := img.(interface {
		...
		tmp := image.NewRGBA(image.Rect(0, 0, cropWidth, cropHeight))
		draw.Draw(tmp, tmp.Bounds(), img, cropRect.Min, draw.Src)
		cropped = tmp
	}

	// Resize to 480x800
	resized := transform.Resize(cropped, 480, 800, transform.Linear)

	// Convert to greyscale for e-ink display
	greyscale := effect.Grayscale(resized)

	// Convert to BMP
	return encodeBMP(greyscale)
}

// CreateImagePreview creates a scaled preview image for crop selection
func CreateImagePreview(imgData []byte, maxWidth, maxHeight int) ([]byte, error) {
	img, _, err := image.Decode(bytes.NewReader(imgData))
	if err != nil {
		return nil, errors.New("failed to decode image")
	}
	...
	if err != nil {
		return nil, errors.New("failed to encode preview image")
	}

	return buf.Bytes(), nil
}

// CreateWallpaperZip creates a zip file containing BMP wallpapers from multiple images
func CreateWallpaperZip(images [][]byte, filenames []string) ([]byte, error) {
	...
	for i, imgData := range images {
		...
		if i < len(filenames) && filenames[i] != "" {
			...
			name := strings.TrimSuffix(filenames[i], ext)
			filename = name + ".bmp"
		}
		...
		bmpData, err := ConvertImageToWallpaper(imgData, 0, 0, 0, 0)
		if err != nil {
			return nil, fmt.Errorf("failed to convert image %d: %v", i, err)
		}

		_, err = fileWriter.Write(bmpData)
		if err != nil {
			return nil, fmt.Errorf("failed to write image %d to zip: %v", i, err)
		}
	}
