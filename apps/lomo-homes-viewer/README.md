...
Lomography protects those pages with a Cloudflare managed challenge, so ordinary Node.js and Python HTTP clients receive HTTP 403 responses. The Compose stack runs a private [FlareSolverr](https://github.com/FlareSolverr/FlareSolverr) sidecar, which uses Chromium to solve the challenge and return the rendered HTML to the scraper scripts.
...
5. All scripts use `scripts/lomography_client.py`, which serializes access to one persistent FlareSolverr browser session so clearance cookies are reused safely
6. The Python scripts extract thumbnail URLs and links from the rendered HTML and return them as JSON
7. The frontend receives the data and displays it in the appropriate view

```
Browser ──GET /api/photos?input=aciano&page=1──▶ Next.js server
                                                    │ execFileSync → Python
                                                    │ POST → private FlareSolverr
                                                    │ Chromium → Lomography
                                                    ▼
...
### Cloudflare sessions and rate limiting

The shared Python client and scrapers include several safeguards:

- **Persistent browser session** — FlareSolverr retains Cloudflare clearance cookies for 30 minutes
- **Cross-process lock** — grid, album, and lightbox scrapers serialize browser navigation instead of racing one Chromium session
- **Session recovery** — an expired or missing FlareSolverr session is recreated and retried once
- **Explicit failure responses** — unsolved challenges and unavailable sidecars produce HTTP 502 responses instead of appearing as empty profiles
- **End-of-album detection** — stops fetching when a page returns empty after images were already found
- **Tolerates empty pages** — stops only after 3 consecutive pages with no images
- **Extended route timeouts** — allow the initial browser challenge to complete
...
|----------|---------|-------------|
| `FLARESOLVERR_URL` | unset | FlareSolverr API URL. Compose sets this to `http://lomo-flaresolverr:8191/v1`. When unset, scripts use direct HTTP for local parser development. |
| `FLARESOLVERR_SESSION` | `lomo-homes-viewer` | Persistent browser session name. |
| `FLARESOLVERR_TIMEOUT_SECONDS` | `60` | Maximum time for one browser navigation. |
| `FLARESOLVERR_SESSION_TTL_MINUTES` | `30` | Browser-session rotation interval. |
...
The app image installs Python 3 for the scraper scripts. Compose also starts the pinned FlareSolverr image on the private backend network; port 8191 is not published to the host or internet.
