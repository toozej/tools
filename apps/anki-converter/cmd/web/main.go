...
func main() {
	...
	err := app.GenerateStaticWebsite(".", &app.Handler{
		Name:        "anki-converter",
		Description: "Convert Anki flashcard decks to e-ink optimised EPUB files",
		Author:      "James Tooze",
		Keywords:    []string{"Anki", "EPUB", "Flashcards", "E-Ink", "WASM", "Go"},
		Styles: []string{
			"/static/app.css",
		},
		Icon: app.Icon{
		...
		app.Div().Class("form-group").Body(
			...
			app.Div().Class("url-row").Body(
				app.Input().
					...
					Class("form-input").
					Type("url").
					Placeholder("https://example.com/my-deck.apkg").
					Value(h.fileURL).
...
func (h *home) onConvert(ctx app.Context, e app.Event) {
	...
	title := strings.TrimSuffix(h.fileName, ".apkg")
	if title == "" {
		title = "Anki Deck"
	}
	...
	ctx.Async(func() {
		...
		ctx.Dispatch(func(ctx app.Context) {
			...
			if valErr := services.ValidateCardCount(result); valErr != nil {
